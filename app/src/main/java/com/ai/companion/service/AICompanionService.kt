...
class AICompanionService : Service() {
    ...
    override fun onCreate() {
        ...
        audioRecorder?.setCallback(object : AudioRecorder.AudioCallback {
            override fun onAudioData(data: ByteArray) {
                ...
                // Process audio every ~5 seconds worth of chunks
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        Log.d(TAG, "Service started - recording")
        return START_STICKY
    }
    ...
    private fun processAudio() {
        ...
        serviceScope.launch {
            try {
                ...
                // In production, this would call a proper STT API
                val audioBase64 = android.util.Base64.encodeToString(combined, android.util.Base64.NO_WRAP)

                // For demo purposes, we simulate periodic processing
                delay(1000)

                // Use AI to generate a suggestion based on the scene
                val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                val sceneDetectionEnabled = prefs.getBoolean(KEY_SCENE_DETECTION, true)

                if (sceneDetectionEnabled) {
                    currentScene = sceneDetector?.detectScene("[audio segment]") ?: SceneType.UNKNOWN
                } else {
                    currentScene = SceneType.DAILY_CHAT
                }

                val sceneContext = "Scene: ${currentScene.name}. " +
                    "The user is in a ${currentScene.name.lowercase().replace("_", " ")} setting."

                val suggestion = aiService?.generateSuggestion(
                    transcript = "[Audio captured: ${totalSize} bytes]",
